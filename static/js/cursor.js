...
if (dot && outline && window.innerWidth > 768) {
    ...
    document.querySelectorAll('a, button, .service-card, .gallery__item').forEach(el => {
        el.addEventListener('mouseenter', () => { outline.style.width = '48px'; outline.style.height = '48px'; outline.style.borderColor = '#F5C6D0'; dot.style.transform = 'scale(1.5)'; });
        el.addEventListener('mouseleave', () => { outline.style.width = '32px'; outline.style.height = '32px'; outline.style.borderColor = '#C9A96E'; dot.style.transform = 'scale(1)'; });
    });
}
