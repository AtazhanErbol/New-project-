...
## Запуск по IP (пока домен ещё не активен)

Если домен `devbench.shop` ещё не активировался, можно запустить сайт сразу
по IP-адресу сервера, а домен подключить позже. Отличия от шагов выше:

- **Шаг 1 (DNS)** — пропускаешь.
- **Шаг 5 (.env)** — вместо домена ставишь IP и выключаешь HTTPS:
  ```
  ALLOWED_HOSTS=IP_ТВОЕГО_СЕРВЕРА
  SECURE_SSL_REDIRECT=False
  ```
- **Шаги 2–8** — делаешь как обычно (nginx-конфиг подойдёт как есть — он
  единственный включённый сайт, поэтому отвечает и на IP).
- **Шаг 9 (certbot/SSL)** — пропускаешь (SSL без домена не выдаётся).

Открываешь сайт: `http://IP_ТВОЕГО_СЕРВЕРА`

### Когда домен станет активен — подключить его
...
Поменяй две строки:
```
ALLOWED_HOSTS=beauty.devbench.shop,IP_ТВОЕГО_СЕРВЕРА
...
certbot --nginx -d beauty.devbench.shop
```
Certbot настроит https. Готово — сайт на `https://beauty.devbench.shop`.
